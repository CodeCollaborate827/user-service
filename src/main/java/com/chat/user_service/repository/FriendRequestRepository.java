...
public interface FriendRequestRepository extends R2dbcRepository<FriendRequest, UUID> {
  ...
  @Query(
      """
          ...
          """)
  Flux<FriendRequestDTO> findFriendRequestOfUserPaging(
      UUID userId, int offset, int limit, FriendRequest.Status status);

  Mono<Integer> countByRecipientIdAndStatus(UUID userId, FriendRequest.Status status);

  Mono<FriendRequest> findBySenderIdAndRecipientIdAndStatus(
      UUID senderId, UUID recipientId, FriendRequest.Status status);
}
